...
/// Converts byte size to human-readable format with appropriate units (B, kB, MB, GB, etc.).
...
/// Deterministically selects a color for user avatar/name based on user ID.
/// Uses modulo operation to ensure consistent color assignment across app sessions.
Color getUserAvatarNameColor(ChatUser user, List<Color> colors) =>
    colors[user.id.hashCode % colors.length];

/// Extracts user initials from first and last name for avatar display.
/// Returns up to 2 uppercase letters, prioritizing first name, then last name.
String getUserInitials(ChatUser user) {
  var initials = '';
...
/// Combines user's first and last name into a single display string.
/// Handles null values gracefully and trims extra whitespace.
String getUserName(ChatUser user) {
  return '${user.firstName ?? ''} ${user.lastName ?? ''}'.trim();
}

/// Formats datetime for display as date dividers in chat history.
/// Shows only time for today's messages, full date and time for older messages.
/// Supports custom date/time formats and localization.
String formattedDateTimeRep(
  DateTime dateTime, {
...
/// Determines if a text message contains only emoji characters for enlargement.
/// Uses comprehensive Unicode regex to detect emoji patterns including complex multi-part emojis.
/// Behavior depends on EmojiEnlargementBehavior setting (single vs multiple emoji support).
bool consistsOfEmojis(
  ...
  final singleEmojiRegExp = RegExp(
    multiEmojiRegExp.pattern.replaceFirst(r'+$', r'$'),
  );
...
/// Platform detection utility to determine if running on mobile device.
/// Used throughout the app for responsive design and platform-specific behaviors.
final isMobile =
    defaultTargetPlatform == TargetPlatform.android ||
    defaultTargetPlatform == TargetPlatform.iOS;

/// Core function that transforms raw messages into a complete chat display structure.
/// Adds date headers, message spacers, unread indicators, and builds image gallery.
/// Handles message grouping, user name display logic, and chronological organization.
List<Object> calculateChatMessages(
  ...
  for (var i = messages.length - 1; i >= 0; i--) {
    ...
    if (!nextMessageInGroup) {
      chatMessages.insert(0, MessageSpacer(height: 12, id: message.id));
    }

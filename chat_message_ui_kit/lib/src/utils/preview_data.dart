...
Future<PreviewData> getPreviewData(
  ...
  try {
    ...
    if (imageRegexp.hasMatch(response.headers['content-type'] ?? '')) {
      ...
      return PreviewData(image: previewDataImage, link: previewDataUrl);
    }
...
Future<PreviewSize> _getImageSize(String url) {
  final completer = Completer<PreviewSize>();
...
@immutable
class PreviewSize {
  /// Creates [PreviewSize] from width and height.
  const PreviewSize({required this.height, required this.width});
